...
rolling_window = st.sidebar.slider("Rolling Window (Observations)", 20, 250, 60)
show_causal = st.sidebar.checkbox("Show Lookahead-Free Expanding Window Causal Z-Score", value=True)
...
base_tranche = st.sidebar.number_input("Base Tranche Size", min_value=0.0, value=10000.0, step=500.0)

st.sidebar.header("👑 Batch Scanner")
batch_mode = st.sidebar.checkbox("Run Sovereign Core batch scan", value=False)
