...
total_portfolio_value = st.sidebar.number_input(
    "Total portfolio value", min_value=0.0, value=1000.0, step=1000.0,
...
with tab_grid:
    ...
    st.dataframe(fixed_display, width="stretch", hide_index=True)
    ...
    if not grid_run_clicked:
        ...
        st.dataframe(format_results(results_df), width="stretch", hide_index=True)

        failed = results_df[results_df["Notes"] != ""]
        if not failed.empty:
            with st.expander(f"⚠️ {len(failed)} ticker(s) had data issues"):
                st.dataframe(failed[["Ticker", "Notes"]], width="stretch", hide_index=True)
